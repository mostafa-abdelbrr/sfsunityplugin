...
namespace SmartFoxClientAPI
{
    ...
    public class SmartFoxClient : IDisposable {
        ...
        /**
         * <summary><see cref="Connect(IPAddress, int)"/></summary>
         */
        public void Connect(IPAddress ipAdr)
        {
            Connect(ipAdr, 9339);
        }
         ...
         * </summary>
         * 
         * <param name="ipAdr">the SmartFoxServer ip address.</param>
         * <param name="port">the SmartFoxServer TCP port (optional)</param>
         * 
         * <example>The following example shows how to connect to SmartFoxServer.
         * 			<code>
         * 			smartFox.Connect(IPAddress.Parse("127.0.0.1"), 9339);
         * 			</code>
         * </example>
         * 
         * <seealso cref="Disconnect"/>
		...
		public void LoadConfig(string configFile, bool autoConnect) {
			...
			try {
				Stream strm = client.OpenRead(configFile);
			...
			xmlDoc.LoadXml(configFileData);

			this.ipAddress = IPAddress.Parse(XmlUtil.GetString(xmlDoc, "./ip"));
			this.blueBoxIpAddress = ipAddress.ToString();			
			this.port = XmlUtil.GetInt(xmlDoc, "./port");
			this.defaultZone = XmlUtil.GetString(xmlDoc, "./zone");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./blueBoxIpAddress") != null )
				this.blueBoxIpAddress = XmlUtil.GetString(xmlDoc, "./blueBoxIpAddress");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./blueBoxPort") != null )
				this.blueBoxPort = XmlUtil.GetInt(xmlDoc, "./blueBoxPort");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./debug") != null )
				this.debug = XmlUtil.GetBool(xmlDoc, "./debug");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./smartConnect") != null )
				this.smartConnect = XmlUtil.GetBool(xmlDoc, "./smartConnect");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./httpPort") != null )
				this.httpPort = XmlUtil.GetInt(xmlDoc, "./httpPort");

			if ( XmlUtil.GetSingleNode(xmlDoc, "./httpPollSpeed") != null )
				SetHttpPollSpeed(XmlUtil.GetInt(xmlDoc, "./httpPollSpeed"));

			if ( XmlUtil.GetSingleNode(xmlDoc, "./rawProtocolSeparator") != null )
				SetRawProtocolSeparator(XmlUtil.GetString(xmlDoc, "./rawProtocolSeparator"));
		...
		public void ProcessEventQueue() {
			...
			Queue sfsQueuedEventsClone = null;
			lock ( sfsQueuedEventsLocker ) {
				sfsQueuedEventsClone = sfsQueuedEvents.Clone() as Queue;
				sfsQueuedEvents.Clear();
			}
			// Now empty our copy
			while ( sfsQueuedEventsClone.Count > 0 ) {
				SFSEvent evt = sfsQueuedEventsClone.Dequeue() as SFSEvent;
				_DispatchEvent(evt);
			}
		}
		...
		public void ProcessSingleEventInEventQueue() {
			...
			lock ( sfsQueuedEventsLocker ) {
				if ( sfsQueuedEvents.Count > 0 ) {
					evt = sfsQueuedEvents.Dequeue() as SFSEvent;
				}
			}
			if ( evt != null ) {
				_DispatchEvent(evt);
		 ...
		 * </summary>
		 * 
		 * <param name="buddy">a buddy object taken from the <see cref="buddyList"/> hashtable.</param>
		 * 
		...
		/**
         * <summary><see cref="CreateRoom(NewRoomDescriptor, int)"/></summary>
         */
		...
		/**
         * <summary><see cref="CreateRoom(Hashtable, int)"/></summary>
         */
		 ...
		 * </item><item>
		 * <term>uCount</term>
		 * <description>(<b>bool</b>) if <c>true</c>, the new room will receive the <see cref="SFSEvent.OnUserCountChangeDelegate"/> notifications (optional, default <u>recommended</u> value: <c>false</c>).</description>
		 * </item><item>
		 * <term>vars</term>
		 * <description>(<b>Hashtable</b>) an hashtable of Room Variables, as described in the <see cref="SetRoomVariables(ArrayList, int, bool)"/> method documentation (optional, default: none).</description>
		 * </item><item>
		 * <term>extension</term>
		...
		public void CreateRoom(Hashtable roomObj, int roomId)
        {
			...
			if (roomObj.ContainsKey("isGame")) {
				isGame = (bool)roomObj["isGame"] ? "1" : "0";
            }
            ...
            if (isGame == "1" && roomObj.ContainsKey("exitCurrent"))
            ...
            else
            {
                ...
                foreach (RoomVariable rv in (ArrayList)roomObj["vars"])
                {
                    xmlMsg += GetXmlRoomVariable(rv);
                }
		 ...
		 * @private
		 */
		public void SetRoomList(Hashtable roomList) 	{
         ...
         * </summary>
         * 
         * <param name="xtName">the name of the extension (see also the <see cref="CreateRoom(Hashtable, int)"/> method).</param>
        ...
        public void SendXtMessage(string xtName, string cmd, ICollection paramObj, string type, int roomId)
        {
            ...
            {
                if (paramObj.GetType() != typeof(ArrayList))
                {
            ...
            if (paramObj == null && (type == SmartFoxClient.XTMSG_TYPE_JSON || type == SmartFoxClient.XTMSG_TYPE_XML)) paramObj = new Hashtable();
            if (paramObj == null && type == SmartFoxClient.XTMSG_TYPE_STR) paramObj = new ArrayList(); 
            ...
            if (type == XTMSG_TYPE_XML)
            {
                ...
                xtReq.Put("cmd", cmd);
                xtReq.PutDictionary("param", (Hashtable)paramObj);
            ...
            else if (type == XTMSG_TYPE_STR)
            {
                ...
                foreach (Object param in paramObj)
		...
		/**
         * <summary><see cref="SetUserVariables(Hashtable, int)"/></summary>
         */
		...
		private void XmlReceived(string msg) {
			...
			try {
				...
				xmlData.LoadXml(msg);
				string handlerId = XmlUtil.GetString(xmlData, "/msg/@t");
		...
		private string MakeXmlHeader(Hashtable headerObj) {
			...
			IDictionaryEnumerator _enumerator = headerObj.GetEnumerator();
			while ( _enumerator.MoveNext() ) {
				xmlData += " " + _enumerator.Key + "='" + _enumerator.Value + "'";
			}
		...
		private void HandleSocketDisconnection() {
			...
			// Fire event
			SFSEvent sfse = new SFSEvent(SFSEvent.onConnectionLostEvent, new Hashtable());
			DispatchEvent(sfse);
		}
		...
		private void HandleSocketData() {
			...
			try {
				while ( true ) {
					...
					AddToMessageBuffer(byteBuffer, bytesRead);
					ArrayList messages = SplitBufferMessages();
										
					if (messages.Count!=0)  {
						bool restsPartOfBuffer = false;
						
						for (int strCount = 0; strCount < messages.Count; strCount++) {
							// If this is the last string and its null, then we send all - nothing left for the buffer
							// place rest in buffer else
							byte [] buf = (byte[])messages[strCount];
							if (strCount == messages.Count-1) {
								if (buf.Length != 0) {
									messageBuffer = buf;
                            ...
                            {
                                HandleMessage(Encoding.UTF8.GetString(buf, 0, buf.Length));
                            }
						}
					} else {
...
//		private string TraceBuffer(byte[] buf, int count) {
//			string str ="";
//			
//			for (int i=0;i<count;i++) {
//				str+=Convert.ToString(buf[i])+"_";
//			}
//			
//			return str;
// 		}
		...
		private void AddToMessageBuffer(byte[] buf, int count) {
			byte [] newBuffer = new byte[messageBuffer.Length + count];
			Buffer.BlockCopy(messageBuffer, 0, newBuffer, 0, messageBuffer.Length);
			Buffer.BlockCopy(buf, 0, newBuffer, messageBuffer.Length, count);
            ...
            try
            {
                switch (evt.GetType())
                {
                    ...
                    case SFSEvent.onBuddyListEvent:
                        ...
                        {
							SFSEvent.onBuddyList(( (SyncArrayList)evt.GetParameter("list") ).ToArrayList());
						}
                        else
                        {
                    ...
                    case SFSEvent.onBuddyRoomEvent:
                        ...
                        {
                            SFSEvent.onBuddyRoom(((SyncArrayList)evt.GetParameter("idList")).ToArrayList());
                        }
                        else
                        {
                    ...
                    case SFSEvent.onRoomListUpdateEvent:
                        ...
                        {
                            SFSEvent.onRoomListUpdate((Hashtable)evt.GetParameter("roomList"));
                        }
                        else
                        {
                    ...
                    case SFSEvent.onRoomVariablesUpdateEvent:
                        ...
                        {
                            SFSEvent.onRoomVariablesUpdate((Room)evt.GetParameter("room"), (Hashtable)evt.GetParameter("changedVars"));
                        }
                        else
                        {
